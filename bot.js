        ...
        if (kufur.some(word => newMsg.content.toLowerCase().includes(word))) {
          try {
            if(!newMsg.member.roles.cache.has(db.fetch(`engellenmeyecekrol${newMsg.guild.id}`)) && !newMsg.member.hasPermission('ADMINISTRATOR')) {
                  newMsg.delete();   
                      return oldMsg.reply('Mesajı Düzenlediğini Yakaladım! Küfür yasak.').then(jkood => jkood.delete({timeout: 5000}))
            }              
...
message.reply('Reklamları engelliyorum!').then(jkood => jkood.delete({timeout: 7000}))
}
});
...
oldMsg.reply('Mesajı Düzenlediğini Yakaladım! Reklam Yapmak Yasak.').then(jkood => jkood.delete({timeout: 7000})) 
}
});
      ...
      if (newMsg.content == caps) {
        if(!newMsg.member.roles.cache.has(db.fetch(`engellenmeyecekrol${newMsg.guild.id}`)) && !newMsg.member.hasPermission('ADMINISTRATOR')) {
          if (!oldMsg.mentions.users.first()) {
            newMsg.delete();
            return oldMsg.channel.send(`${oldMsg.member}, Mesajı Düzenlediğini Yakaladım! CapsLock Kullanımı Yasak.`).then(nordx => nordx.delete({timeout: 5000}))
